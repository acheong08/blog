...
- Decrypt your partition
  `cryptsetup luksOpen /dev/nvme0n1pX encrypted_partition` (find it from
  `lsblk`)
...
You can, however, check the installed files against what is expected - with some
...
- Once you have the root partition mounted, also mount the boot partition. In my
  case `mount /dev/nvme0n1p1 /mnt/boot`.
- Then `arch-chroot /mnt` and `pacman -S linux` regenerated the missing
  `initramfs-linux.img`.
