...
branch managed by `ostree`, meaning that the user is **not** supposed to make
**any** changes to it by hand. As everything is kept track by `ostree`, you can
always roll back to the previous installation.
...
Looking at the manual for `ostree fsck`, I found the `--delete` option... So I
ran it.
...
`fedora:fedora/38/x86_64/testing/silverblue`. After getting a working system,
you can pull `fedora:fedora/38/x86_64/silverblue` again. {: .notice--warning}
...
**Warning** You might lose some installed packages. {: .notice--warning}
